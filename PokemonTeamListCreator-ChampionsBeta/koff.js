// Add the formula at the top to make it globally available to the parser
export const FORMULA = {
  'to-new': { label:'→ Champions', fn: v => Math.round((v + 4) / 8) },
  'to-old': { label:'→ Old Format', fn: v => (v === 0 ? 0 : v * 8 - 4) } 
};
...
var Pokemon = class {
  ...
  static fromObject(obj) {
    ...
    p.level = 50; // Locked at 50
    p.shiny = obj.shiny;
    p.happiness = obj.happiness;
    p.nature = obj.nature;
    p.evs = obj.evs;
    p.championsEvs = obj.championsEvs;
    p.ivs = obj.ivs;
  ...
  toShowdown() {
    ...
    // Use championsEvs for the text output if it exists, otherwise fallback
    if (this.championsEvs) {
      const evs = this.championsEvs;
...
var _ShowdownParser = class {
  ...
  _parseEvsIvs(line, pokemon) {
    ...
    if (line.match(rg.eivs)) {
      ...
      values.forEach(function(stat) {
        ...
        let rawValue = parseInt(statData[1]);
        let statName = statData[2];

        if (prop === "evs") {
          // Input EVs are in Champions Format (0-32). Limit them safely.
          let championsEv = clamp(rawValue, 0, 32);
          
          if (!pokemon.championsEvs) pokemon.championsEvs = {};
          pokemon.championsEvs[statName] = championsEv;
          
          // CONVERT TO OLD FORMAT (0-252) FOR THE STAT CALCULATOR!
          // This fixes the low stats issue you showed in the images.
          pokemon.evs[statName] = FORMULA['to-old'].fn(championsEv);
        } else {
          // Normal IVs (0-31)
          pokemon[prop][statName] = clamp(rawValue, 0, 31);
        }
      });
      return true;
    }
  ...
  _parseKeyValuePairs(line, pokemon) {
    ...
    return propNames.some(function(key) {
      ...
      // Strict Overrides
      if (key === "teraType") {
        return true; // Ignore Tera Types entirely
      }
      if (key === "level") {
        return true; // Ignore pasted level, keeps default 50
      }
